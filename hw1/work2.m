...
grid on;                                                    % 形成网格
hold on;                                                    % 保持图形
title('真空中两个等量异性点电荷的等电位线(11910103 秦庆福)','fontsize',20);    % 绘制出图形标题
xlabel('X轴(单位：m)','fontsize',15);                       % 绘制出X轴标注
ylabel('Y轴(单位：m)','fontsize',15);                       % 绘制出Y轴标注
hold off;                                                   % 取消保持
...
title('真空中两个等量异性点电荷的等电位线及电场线（用光滑连续曲线表示）(11910103 秦庆福)','fontsize',20);    % 绘制出图形标题
...
hold on;                                                    % 保持图形
contour(X,Y,V,Veq);                                         % 绘制等势线
title('真空中两个等量异性点电荷的等电位线及电场线（用归一化箭头表示）(11910103 秦庆福)','fontsize',20);    % 绘制出图形标题
xlabel('X轴(单位：m)','fontsize',15);                       % 绘制出X轴标注
ylabel('Y轴(单位：m)','fontsize',15);                       % 绘制出Y轴标注
hold off;                                                   % 取消保持
