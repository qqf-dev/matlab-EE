...
k=9e9;                                                      % 设置静电力衡量
Q1=5e-9;                                                    % 设置点电荷q1的电量
Q2=-5e-9;                                                   % 设置点电荷q2的电量，大小与q1相同，但带异种电荷
% 绘制场域内各点电位分布
xm=8;                                                    % 设置场域中x方向的范围
ym=8;                                                    % 设置场域中y方向的范围
x=linspace(-xm,xm,60);                                      % 将x轴等分成60等份
y=linspace(-ym,ym,60);                                      % 将y轴等分成60等份
[X,Y]=meshgrid(x,y);                                        % 形成场域中各点的坐标
R1=sqrt((X+2).^2+Y.^2);                                  % 计算场域中各点到点电荷q1的距离（到点p1（-2，0）的距离）
R2=sqrt((X-2).^2+Y.^2);                                  % 计算场域中各点到点电荷q2的距离（到点p2（2，0）的距离）
...
hold on;                                                    % 保持图形                                                 
title('真空中两个等量异性点电荷的电场线','fontsize',20);      % 绘制出图形标题
xlabel('X轴(单位：m)','fontsize',15);                       % 绘制出X轴标注
ylabel('Y轴(单位：m)','fontsize',15);                       % 绘制出Y轴标注
hold off;                                                   % 取消保持
% 绘制场域内等电位线分布
Vmin=-30;                                                   % 设置等位线族的最小电位值
Vmax=30;                                                  % 设置等位线族的最大电位值
Veq=linspace(Vmin,Vmax,30);                                 % 设定30条等位线的电位值
figure(2);                                                  % 在图像2窗口绘图
contour(X,Y,V,Veq);                                         % 绘制20条等势线
contour(X,Y,V,Veq);                                         % 绘制20条等势线
grid on;                                                    % 形成网格
hold on;                                                    % 保持图形
title('真空中两个等量异性点电荷的等电位线','fontsize',20);    % 绘制出图形标题
xlabel('X轴(单位：m)','fontsize',15);                       % 绘制出X轴标注
ylabel('Y轴(单位：m)','fontsize',15);                       % 绘制出Y轴标注
hold off;                                                   % 取消保持
...
xs1=0.1*cos(theta)-2;                                  % 从q1生成电力线的x轴坐标
...
plot(-2,0,'ro','MarkerSize',12);                         % 在p1画出点电荷
plot(2,0,'bo','MarkerSize',12);                          % 在p2画出点电荷
title('真空中两个等量异性点电荷的等电位线及电场线（用光滑连续曲线表示）','fontsize',20);    % 绘制出图形标题
...
hold on;                                                    % 保持图形
contour(X,Y,V,Veq);                                         % 绘制等势线
title('真空中两个等量异性点电荷的等电位线及电场线（用归一化箭头表示）','fontsize',20);    % 绘制出图形标题
xlabel('X轴(单位：m)','fontsize',15);                       % 绘制出X轴标注
ylabel('Y轴(单位：m)','fontsize',15);                       % 绘制出Y轴标注
hold off;                                                   % 取消保持
