...
theta0 = linspace(0, 2 * pi, N + 1); %Division ofthe angle of circumference
theta1 = theta0(1:N);
x1 = a * cos(theta1);
y1 = a * sin(theta1); %Thestartpoint coordinateof eachsegment of the loop
theta2 = theta0(2:N + 1);
x2 = a * cos(theta2);
y2 = a * sin(theta2); %The endingpoint coordinate of each segment of the loop
zc = 0; xc = (x2 + x1) ./ 2;
yc = (y2 + y1) ./ 2; %Calculate the 3 coordinate components of the midpoint of
%each segment of the loop.
dlz = 0;
dlx = x2 - x1;
dly = y2 - y1; %Calculate the 3 lengthcomponents of each segment
%vector dl.
NGx = 20;
NGy = 20; %Griddimension
Hy = zeros(20);
Hz = zeros(20);
H = zeros(20); %Construct the H matrix

for i = 1:NGy%Loop computation of thevalue of H(x,y) in each grid

    for j = 1:NGx
        rx = 0 - xc;
        ry = y(j) - yc;
        rz = z(i) - zc; %Calculate the 3 length components of the radius vector r,
...
subplot(1, 3, 2), mesh(y, z, H); %Plot the graph of magneticfield intensity
axis([-3, 3, -3, 3, 0, 100])
xlabel('y'), ylabel('z'), zlabel('H');
theta = [0 50 60 70 80 90 100 110 120 130 180] .* pi / 180; %Set the radian value of the streamlines
ys = 1.1 * cos(theta); %Set thestreamline startingcircle’s y coordinate
zs = 1.1 * sin(theta); %Set the streamline startingcircle’s z coordinate
subplot(1, 3, 3), streamline(y, z, Hy, Hz, ys, zs); %Outwardly plot the magnetic line of force from the starting
%circle.
streamline(y, z, -Hy, -Hz, ys, zs); %Inwardly plot the magnetic line of force from the starting
%circle.
xlabel('y'), ylabel('z');
